...
# -*- coding: utf-8 -*-
"""
Final LongRA appendix example figure (two-column safe, no overlap, text fits prompt box).
...
Each sample:
{
  ...
  "a": "Hanoi",
  "b": "Vietnam",
  "prompt": "... I was staying in the capital,",
  "target_token": "Hanoi"
}
"""
...
# Academic-style color scheme (LongRA theme - Purple)
COLORS = {
    'longra': '#9b59b6',             # Purple
    ...
    'answer_accent': '#e74c3c'       # Red accent for target token
}
...
def draw_cards(
    ...
    # Card paddings & fixed blocks (inches)
    card_pad_top_in = 0.20
    card_pad_bot_in = 0.16

    gap_title_meta_in     = 0.08
    gap_meta_pair_in      = 0.08
    gap_pair_prompt_in    = 0.10
    gap_prompt_target_in  = 0.12
    gap_label_value_in    = 0.06

    # Fixed prompt box height (inches) - LongRA tends to be longer
    prompt_box_h_in = 0.78
    ...
    card_h_in = (
        ...
        + 0.20  # meta line
        + gap_meta_pair_in
        + 0.18  # "Entity pair:" label
        + gap_label_value_in
        + 0.22  # pair value
        + gap_pair_prompt_in
        + prompt_box_h_in
        + gap_prompt_target_in
        + 0.18  # "Target token:" label
        + gap_label_value_in
        + 0.22  # target token value
        + card_pad_bot_in
    )
    ...
    for s in samples:
        ...
        accent = Rectangle(
            ...
            facecolor=COLORS["longra"], zorder=3
        )
        ax.add_patch(accent)
        ...
        # 1) Case title
        t = ax.text(content_x, y_in(y), f"Case {sid}",
        ...
        # 4) Entity pair value
        pair = f"a = {a},  b = {b}"
        t = ax.text(content_x, y_in(y), pair,
                    fontsize=fs_value,
                    va="top", ha="left",
                    color=COLORS["text_primary"], zorder=4, clip_on=True)
        t.set_clip_path(card)
        y -= (0.22 + gap_pair_prompt_in)
        ...
        y -= (prompt_box_h_in + gap_prompt_target_in)

        # 6) Target token label
        t = ax.text(content_x, y_in(y), "Target token:",
                    fontsize=fs_label, fontweight="bold",
                    va="top", ha="left",
                    color=COLORS["text_secondary"], zorder=4, clip_on=True)
        t.set_clip_path(card)
        ...
        # 7) Target token value
        t = ax.text(content_x, y_in(y), tgt,
                    fontsize=fs_tgt, fontweight="bold",
                    va="top", ha="left",
