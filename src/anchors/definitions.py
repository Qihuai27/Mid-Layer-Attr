"""
Task configuration and anchor word definitions.
"""
...
@dataclass
class TaskConfig:
    """Configuration for a classification task with anchor word definitions."""
    ...
    anchor_prefix: List[str]  # ["Sentiment", ":"]
...
def load_task_config(task_name: str, config_dir: Optional[str] = None) -> TaskConfig:
    ...
    return TaskConfig(
        ...
        anchor_prefix=config['anchor_pattern']['prefix'],
    )
...
def get_available_tasks(config_dir: Optional[str] = None) -> List[str]:
    ...
    if config_dir is None:
        config_dir = Path(__file__).parent.parent.parent.parent / "configs" / "tasks"
